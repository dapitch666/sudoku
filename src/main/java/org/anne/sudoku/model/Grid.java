...
public class Grid {
    ...
    public Cell[] getCommonPeersWithCandidates(Cell cell1, Cell cell2, List<Integer> digits) {
        List<Cell> commonPeers = new ArrayList<>();
        for (Cell peer : getPeers(cell1)) {
            if (peer.isPeer(cell2) && digits.stream().allMatch(peer::isCandidate)) {
                commonPeers.add(peer);
            }
        }
        return commonPeers.toArray(new Cell[0]);
    }
